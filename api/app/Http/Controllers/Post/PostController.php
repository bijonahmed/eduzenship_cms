...
class PostController extends Controller
{
    ...
    public function save(Request $request)
    {
       // dd($request->all());
        ...
        if (empty($request->id)) {
            if ($request->post_category_id == 8) {
                // First insert with post_category_id = 8
                $data['post_category_id'] = 8;
                $resdata['first_post_id'] = Post::insertGetId($data);

                // Second insert with post_category_id = 14
                $data['post_category_id'] = 14;
                $resdata['second_post_id'] = Post::insertGetId($data);
            } else {
                // Normal insert
    ...
    public function allPostList(Request $request)
    {
        ...
        if ($selectedFilter !== null) {
            $query->where('posts.status', $selectedFilter);
        }
